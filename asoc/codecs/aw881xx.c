...
#define AW881XX_DRIVER_VERSION "v0.2.1.1"
...
int aw881xx_fw_update(struct aw881xx *aw881xx, bool up_dsp_fw_en)
{
	int ret = -1;
...
static void aw881xx_startup_work(struct work_struct *work)
{
	...
	if (aw881xx->fw_status == AW881XX_FW_OK) {
		...
		if (ret < 0) {
			aw_dev_err(aw881xx->dev, "%s:check dsp status failed\n",
				__func__);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (aw881xx->pstream) {
		if (ucontrol->value.integer.value[0] == 0) {
			...
			cancel_delayed_work_sync(&aw881xx->start_work);
			queue_delayed_work(aw881xx->work_queue, &aw881xx->start_work, 0);
		}
	} else {
		mutex_lock(&aw881xx->lock);
...
static int aw881xx_check_cfg_by_hdr(struct aw881xx_container *aw_cfg)
{
	...
	act_data += sizeof(struct aw_cfg_hdr) + cfg_hdr->a_ddt_size;
		...
		struct aw_cfg_hdr *prof_hdr, struct aw_all_prof_info *all_prof_info)
{
	...
	for (i = 0; i < prof_hdr->a_ddt_num; i++) {
		if ((aw881xx->i2c->adapter->nr == cfg_dde[i].dev_bus) &&
			...
			if (ret < 0) {
				aw_dev_err(aw881xx->dev, "%s:parse bus[0x%x] addr[0x%x] scene[%s] data failed\n",
					__func__, aw881xx->i2c->adapter->nr, aw881xx->i2c->addr,
	...
	for (i = 0; i < prof_hdr->a_ddt_num; i++) {
		if ((aw881xx->channel == cfg_dde[i].dev_index) &&
			...
			ret = aw881xx_parse_data_by_sec_type(aw881xx, prof_hdr, &cfg_dde[i],
					&all_prof_info->prof_desc[cfg_dde[i].dev_profile]);
			if (ret < 0) {
				aw_dev_err(aw881xx->dev, "%s: parse index[%d] scene[%s] data failed\n",
					__func__, cfg_dde[i].dev_index, profile_name[cfg_dde[i].dev_profile]);
				return ret;
			}
			sec_num++;
		}
...
static void aw881xx_request_firmware_file(struct aw881xx *aw881xx)
{
	...
	ret = aw881xx_load_chip_profile(aw881xx);
	if (ret < 0)
		aw_dev_err(aw881xx->dev, "%s:cfg loading requested failed: %d",
			__func__, ret);

	ret = aw881xx_load_monitor_profile(&aw881xx->monitor);
	if (ret < 0)
		aw_dev_err(aw881xx->dev, "%s:loading monitor cfg failed: %d",
			__func__, ret);
				...
				struct device_node *np)
{
	...
	aw881xx_parse_monitor_dt(&aw881xx->monitor);
...
static int aw881xx_hw_reset(struct aw881xx *aw881xx)
{
	...
	if (gpio_is_valid(aw881xx->reset_gpio)) {
		gpio_direction_output(aw881xx->reset_gpio, 0);
		msleep(1);
		gpio_direction_output(aw881xx->reset_gpio, 1);
		msleep(1);
	} else {
...
static int aw881xx_ui_update_dsp(struct aw881xx *aw881xx)
{
	...
	for (i = 0; i < iis_check_max; i++) {
		ret = aw881xx_get_iis_status(aw881xx);
		if (ret < 0) {
			aw_dev_err(aw881xx->dev, "%s: get no iis signal, ret=%d\n",
				__func__, ret);
			msleep(2);
		} else {
			break;
				...
				const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(aw881xx->reset_gpio)) {
		...
		ret = gpio_request(aw881xx->reset_gpio, aw881xx_rst);
		if (ret) {
	...
	if (gpio_is_valid(aw881xx->irq_gpio)) {
		...
		ret = gpio_request( aw881xx->irq_gpio,aw881xx_int);
		if (ret) {
