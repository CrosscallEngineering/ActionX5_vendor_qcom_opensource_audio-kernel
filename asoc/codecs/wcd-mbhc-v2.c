				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
      ...
      pr_info("%s:jack_type = %d, is_pa_on= %d \n",	__func__, jack_type, is_pa_on);
#ifdef CONFIG_SND_HIFI_ES9218		 
        if (mbhc->impedance_detect &&
			mbhc->mbhc_cb->compute_impedance &&
			(mbhc->mbhc_cfg->linein_th != 0) &&
			(!is_pa_on)&&
			(!es9218_hph_switch_get())) {
#else
        if (mbhc->impedance_detect &&
			mbhc->mbhc_cb->compute_impedance &&
			(mbhc->mbhc_cfg->linein_th != 0) &&
			(!is_pa_on)) {			   
			    ...
			    pr_info("%s:hph res calc end \n", __func__);
		}
#ifdef CONFIG_SND_HIFI_ES9218					 
		  else{
			  pr_info("%s: 200ms sleep \n", __func__);
			   msleep(200);
		  }
#endif	
