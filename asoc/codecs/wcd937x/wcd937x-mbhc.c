...
#ifdef CONFIG_SND_HIFI_ES9218
	int32_t z1L_bak; /*hmct added*/ 
#endif	
	struct wcd937x_mbhc_zdet_param zdet_param[] = {
...
#ifdef CONFIG_SND_HIFI_ES9218	
	z1L_bak = z1L;
	if ((z1L_bak == WCD937X_ZDET_FLOATING_IMPEDANCE)||
		(z1L_bak > WCD937X_ZDET_VAL_100K)){
		dev_dbg(component->dev, "%s: impedance on z1L_bak = %d(ohms)\n",
		__func__, z1L_bak);
		z1L_bak = WCD937X_ZDET_FLOATING_IMPEDANCE;	
	}	
#endif
/*hmct added, end*/
...
/*hmct added, for selfie stick check*/
#ifdef CONFIG_SND_HIFI_ES9218	
    if ((*zl == 0)&&
        (z1L_bak == WCD937X_ZDET_FLOATING_IMPEDANCE)&&
        (*zr == 0)) {  /*hmct added, for selfie stick check*/
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
    ...
    if((zr > 5)&&(zr< 10000))
    {
        dev_err(component->dev, "%s:original value  zl=%u(ohms), zr=%u(ohms)\n", __func__,
		zl, zr);
	    ucontrol->value.integer.value[0] = hphr ? zr : zl;

    	return 0;
	}
    is_pa_on = wcd937x_mbhc_hph_pa_on_status(component);
    if ((!is_pa_on)&&
	   (wcd937x_mbhc->wcd_mbhc.is_hs_inserted)){
	   if ((snd_soc_component_read32(component, WCD937X_ANA_MBHC_ELECT) &
			0x80))
		   fsm_en =1;
	   else
		   fsm_en =0;
       /* read MUX_CTL*/
       mux_ctl = snd_soc_component_read32(component, WCD937X_MBHC_NEW_CTL_2);
	   /* Set MUX_CTL to AUTO for Z-det */
       /* set fsm_en , 0*/
	   snd_soc_component_update_bits(component, WCD937X_ANA_MBHC_ELECT,
				    0x80, 0x00);
       /* set  MUX_CTL, MUX_CTL_AUTO*/
	   snd_soc_component_update_bits(component, WCD937X_MBHC_NEW_CTL_2,
				    0x70, 0x00);
	   /* set fsm_en , 1*/
	   snd_soc_component_update_bits(component, WCD937X_ANA_MBHC_ELECT,
				    0x80, 0x80);	
	   wcd937x_wcd_mbhc_calc_impedance(&wcd937x_mbhc->wcd_mbhc,
					&zl, &zr);
       if (fsm_en)
		   snd_soc_component_update_bits(component, WCD937X_ANA_MBHC_ELECT,
				    0x80, 0x80);
	   ...
	   snd_soc_component_update_bits(component, WCD937X_MBHC_NEW_CTL_2,
				    0x7f, mux_ctl);
	   wcd937x_mbhc->wcd_mbhc.zl=zl;
	   wcd937x_mbhc->wcd_mbhc.zr=zr;
	   dev_err(component->dev, "%s:caculate value  zl=%u(ohms), zr=%u(ohms)\n", __func__,
		zl, zr);
	}
#endif 
